...
gboolean g_source_hyperdeck_play (hyperdeck_t *hyperdeck)
{
...
gboolean g_source_hyperdeck_stop (hyperdeck_t *hyperdeck)
{
...
gboolean g_source_hyperdeck_loop (hyperdeck_t *hyperdeck)
{
...
gboolean g_source_hyperdeck_single_clip_true_loop_true (hyperdeck_t *hyperdeck)
{
...
gboolean g_source_hyperdeck_single_clip_true_loop_false (hyperdeck_t *hyperdeck)
{
...
gboolean g_source_hyperdeck_single_clip_false_loop_false (hyperdeck_t *hyperdeck)
{
...
gboolean g_source_hyperdeck_single_clip_false_loop_true (hyperdeck_t *hyperdeck)
{
...
gboolean g_source_select_clip (clip_list_t *clip)
{
	LOG_OSC_2_STRINGS("g_source_select_clip () ",clip->name)
...
void parse_osc_hyperdeck (hyperdeck_t *hyperdeck, char *buffer)
{
	...
	if (strcmp (buffer, "Play") == 0) g_idle_add ((GSourceFunc)g_source_hyperdeck_play, hyperdeck);
	else if (strcmp (buffer, "Stop") == 0) g_idle_add ((GSourceFunc)g_source_hyperdeck_stop, hyperdeck);
	else if (strcmp (buffer, "Loop") == 0) g_idle_add ((GSourceFunc)g_source_hyperdeck_loop, hyperdeck);
	else if (strcmp (buffer, "Single_clip_True_Loop_True") == 0) g_idle_add ((GSourceFunc)g_source_hyperdeck_single_clip_true_loop_true, hyperdeck);
	else if (strcmp (buffer, "Single_clip_True_Loop_False") == 0) g_idle_add ((GSourceFunc)g_source_hyperdeck_single_clip_true_loop_false, hyperdeck);
	else if (strcmp (buffer, "Single_clip_False_Loop_False") == 0) g_idle_add ((GSourceFunc)g_source_hyperdeck_single_clip_false_loop_false, hyperdeck);
	else if (strcmp (buffer, "Single_clip_False_Loop_True") == 0) g_idle_add ((GSourceFunc)g_source_hyperdeck_single_clip_false_loop_true, hyperdeck);
	...
	else if (strcmp (buffer, "Up") == 0) g_idle_add ((GSourceFunc)g_source_select_clip_up, hyperdeck);
	else if (strcmp (buffer, "Down") == 0) g_idle_add ((GSourceFunc)g_source_select_clip_down, hyperdeck);
	else {
		for (clip = hyperdeck->list_of_clips; clip != NULL; clip = clip->next) {
			if (strcmp (buffer, clip->name) == 0) {
				g_idle_add ((GSourceFunc)g_source_select_clip, clip);
...
gboolean g_source_fresque_loop (void)
{
...
gboolean g_source_fresque_loop_on (void)
{
...
gboolean g_source_fresque_loop_off (void)
{
...
gboolean g_source_fresque_play (void)
{
...
gboolean g_source_fresque_stop (void)
{
...
gboolean g_source_select_fresque (fresque_t *fresque)
{
	LOG_OSC_2_STRINGS("g_source_select_fresque () ",fresque->name)
...
void parse_osc_fresque (char *buffer)
{
	...
	if (strcmp (buffer, "Loop") == 0) g_idle_add ((GSourceFunc)g_source_fresque_loop, NULL);
	else if (strcmp (buffer, "Loop_On") == 0) g_idle_add ((GSourceFunc)g_source_fresque_loop_on, NULL);
	else if (strcmp (buffer, "Loop_Off") == 0) g_idle_add ((GSourceFunc)g_source_fresque_loop_off, NULL);
	else if (strcmp (buffer, "Play") == 0) g_idle_add ((GSourceFunc)g_source_fresque_play, NULL);
	else if (strcmp (buffer, "Stop") == 0) g_idle_add ((GSourceFunc)g_source_fresque_stop, NULL);
	else if (strcmp (buffer, "Up") == 0) g_idle_add ((GSourceFunc)g_source_select_fresque_up, NULL);
	else if (strcmp (buffer, "Down") == 0) g_idle_add ((GSourceFunc)g_source_select_fresque_down, NULL);
	else {
		for (fresque = fresques; fresque != NULL; fresque = fresque->next) {
			if (strcmp (buffer, fresque->name) == 0) {
				g_idle_add ((GSourceFunc)g_source_select_fresque, fresque);
...
gboolean g_source_select_preset (GtkButton *button)
{
	LOG_OSC_2_STRINGS("g_source_select_preset () ",gtk_button_get_label (button))
...
void parse_osc_preset (char *buffer)
{
	...
	for (i = 0; i < NB_OF_PRESETS; i++) {
		if (strcmp (buffer, default_presets_name[i] + 7) == 0) {
			if (presets[i].switched_on) g_idle_add ((GSourceFunc)g_source_select_preset, presets[i].button);
